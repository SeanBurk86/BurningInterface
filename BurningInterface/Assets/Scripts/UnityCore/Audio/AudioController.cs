...
namespace UnityCore
{
    namespace Audio
    {
        public class AudioController : MonoBehaviour
        {
            ...
            public void PlayAudio(AudioType _type, float _fade=0.0f, float _delay=0.0f)
            {
                AddJob(new AudioJob(AudioAction.START, _type, _fade, _delay));
            }
            
            public void StopAudio(AudioType _type, float _fade=0.0f, float _delay=0.0f)
            {
                AddJob(new AudioJob(AudioAction.STOP, _type, _fade, _delay));
            }
            
            public void RestartAudio(AudioType _type, float _fade=0.0f, float _delay=0.0f)
            {
            ...
            private void Dispose()
            {
                if (m_JobTable == null)
                {
                    LogWarning("JobTable does not have a valid reference");
                    return;
                }
            ...
            private IEnumerator RunAudioJob(AudioJob _job)
            {
                ...
                if (_job.fade>0.0f)
                {
                    float _initial = _job.action == AudioAction.START || _job.action == AudioAction.STOP ? 0.0f : 1.0f;
                    float _target = _initial == 0 ? 1 : 0;
                    float _duration = _job.fade;
                    float _timer = 0.0f;
